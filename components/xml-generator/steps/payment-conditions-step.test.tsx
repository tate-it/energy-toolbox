...
describe('PaymentConditionsStep', () => {
  it('renders the component title and description', () => {
    ...
    expect(screen.getByText('Metodi di Pagamento e Condizioni')).toBeInTheDocument()
    expect(screen.getByText(/Definisci i metodi di pagamento accettati/)).toBeInTheDocument()
  })
  ...
  it('renders payment methods section', () => {
    ...
    expect(screen.getByText(/Specifica i metodi di pagamento disponibili/)).toBeInTheDocument()
  })
  ...
  it('renders contractual conditions section', () => {
    ...
    expect(screen.getByText(/Aggiungi le condizioni contrattuali specifiche/)).toBeInTheDocument()
  })
  ...
  it('shows payment method type options', () => {
    ...
    const paymentTypeSelect = screen.getByRole('combobox', { name: /Tipo di Pagamento/ })
    ...
    expect(screen.getAllByText('Credit card direct debit')[0]).toBeInTheDocument()
  ...
  it('shows description field when "Other" payment method is selected', () => {
    ...
    expect(screen.getByPlaceholderText('Descrivi il metodo di pagamento alternativo...')).toBeInTheDocument()
    expect(screen.getByText('Obbligatorio quando si seleziona "Altro". Massimo 3000 caratteri.')).toBeInTheDocument()
  })
  ...
  it('allows adding new contractual conditions', () => {
    ...
    expect(screen.getByText('Descrizione della Condizione *')).toBeInTheDocument()
  })
  ...
  it('shows condition type options', () => {
    ...
    const conditionTypeSelect = screen.getByRole('combobox', { name: /Tipo di Condizione/ })
    ...
    expect(screen.getAllByText('Early Withdrawal Charges')[0]).toBeInTheDocument()
    expect(screen.getAllByText('Other')[0]).toBeInTheDocument()
  })
  ...
  it('shows limiting condition options', () => {
    ...
    const limitingSelect = screen.getByRole('combobox', { name: /Condizione Limitante/ })
  ...
  it('shows alternative description field when "Other" condition type is selected', () => {
    ...
    expect(screen.getByPlaceholderText('Descrivi il tipo di condizione alternativa...')).toBeInTheDocument()
    expect(screen.getByText('Obbligatorio quando si seleziona "Altro". Massimo 3000 caratteri.')).toBeInTheDocument()
  })
  ...
  it('allows removing contractual conditions', () => {
    ...
    expect(screen.getByText('Condizione Contrattuale 1')).toBeInTheDocument()
    expect(screen.queryByText('Condizione Contrattuale 2')).not.toBeInTheDocument()
  })
  ...
  it('handles form input changes for payment methods', () => {
    ...
    const descriptionInput = screen.getByPlaceholderText('Descrivi il metodo di pagamento alternativo...')
    fireEvent.change(descriptionInput, { target: { value: 'Custom payment method' } })
  ...
  it('handles form input changes for contractual conditions', () => {
    ...
    const altDescriptionInput = screen.getByPlaceholderText('Descrivi il tipo di condizione alternativa...')
    fireEvent.change(altDescriptionInput, { target: { value: 'Custom condition type' } })

    expect(altDescriptionInput).toHaveValue('Custom condition type')

    const descriptionInput = screen.getByPlaceholderText('Descrivi dettagliatamente la condizione contrattuale...')
    fireEvent.change(descriptionInput, { target: { value: 'Detailed condition description' } })
  ...
  it('shows validation help text', () => {
    ...
    const addConditionButton = screen.getByText('Aggiungi Condizione Contrattuale')
