...
import { basicInfoSchema, type BasicInfoFormValues } from '@/lib/xml-generator/schemas'

// Test wrapper component that provides form context
function TestWrapper({ children, defaultValues = {} }: { children: React.ReactNode; defaultValues?: Partial<BasicInfoFormValues> }) {
...
describe('BasicInfoStep', () => {
  ...
  it('shows validation errors for empty fields', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/La PIVA deve contenere almeno 11 caratteri/i)).toBeInTheDocument()
      expect(screen.getByText(/Il codice offerta è obbligatorio/i)).toBeInTheDocument()
    })
  })
  ...
  it('validates PIVA format correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/La PIVA deve contenere solo lettere maiuscole e numeri/i)).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(screen.queryByText(/La PIVA deve contenere solo lettere maiuscole e numeri/i)).not.toBeInTheDocument()
    })
  })
  ...
  it('validates offer code format correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Il codice offerta deve contenere solo lettere maiuscole e numeri/i)).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(screen.queryByText(/Il codice offerta deve contenere solo lettere maiuscole e numeri/i)).not.toBeInTheDocument()
    })
  })
  ...
  it('enforces maximum length limits', async () => {
    ...
    const offerCodeInput = screen.getByLabelText(/Codice Offerta/i) as HTMLInputElement
  ...
  it('displays helpful information about field requirements', () => {
    ...
    expect(screen.getByText(/Partita IVA italiana \(11-16 caratteri alfanumerici\)/i)).toBeInTheDocument()
    expect(screen.getByText(/Codice offerta univoco \(max 32 caratteri alfanumerici\)/i)).toBeInTheDocument()
    expect(screen.getByText(/PIVA_UTENTE: Rappresenta la partita IVA dell'utente accreditato/i)).toBeInTheDocument()
    expect(screen.getByText(/COD_OFFERTA: Codice univoco utilizzato nel campo CODICE CONTRATTO/i)).toBeInTheDocument()
  })
  ...
  it('accepts valid data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText(/Il codice offerta deve/i)).not.toBeInTheDocument()
    })
  })
